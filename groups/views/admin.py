...
def codeCheck(check_student_submission, other_student_submission, rate):
    """
    :return
    ...
    count = 0
    length = len(words1) if len(words1) < len(words2) else len(words2)
...
class CodeDuplicationCheck(APIView):
    def post(self, request):
        ...
        for student in al_achieve_students:
            ...
            for submission in submissions:
                if homework.begin_time < submission.create_time < homework.end_time:
                    info = codeCheck(check_student_submission, submission, rate)
                    if info["isCopy"]:
                        data.append(info)
                    break
                else:
